...
export async function GET(request: NextRequest) {
  try {
    ...
    const db = getDb();
    const rows = await db`
      SELECT id, user_id, blob_path, content_type, status
      FROM clips
    ...
    // Admins can preview clips before approval so they can inspect content safely.
    // Regular players can only view their own approved clips.
    if (!clip || (!admin && (String(clip.user_id) !== String(user.id) || clip.status !== "approved"))) {
      return new NextResponse("Not found.", { status: 404 });
    }

    const result = await get(String(clip.blob_path), {
      access: "private",
