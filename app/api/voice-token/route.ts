...
export async function POST(request: Request) {
  try {
    ...
    const invite = rows[0];
    if (String(invite.caller_id) !== user.id && String(invite.recipient_id) !== user.id) {
      return NextResponse.json({ error: "You are not part of this voice invite." }, { status: 403 });
    }
    if (invite.status !== "accepted") {
      return NextResponse.json({ error: "This voice invite has not been accepted." }, { status: 409 });
    }

    // These values must come from the same LiveKit project. Trim accidental whitespace
    // from Vercel's environment-variable values, but never expose the secret to the client.
    const apiKey = process.env.LIVEKIT_API_KEY?.trim();
    const apiSecret = process.env.LIVEKIT_API_SECRET?.trim();
    const livekitUrl = process.env.LIVEKIT_URL?.trim();
    if (!apiKey || !apiSecret || !livekitUrl) {
      console.error("LiveKit token configuration is incomplete", {
    ...
    if (!jwt || jwt.split(".").length !== 3) {
      console.error("LiveKit SDK returned an invalid JWT");
      return NextResponse.json({ error: "Unable to create a valid LiveKit token." }, { status: 500 });
    }
