...
function buildProfile(user: any, attempts: any[]) {
  ...
  return { name: user.username, createdAt: user.created_at, isAdmin, attempts: attempts.map((attempt, index) => { const expiresAt = new Date(new Date(attempt.date).getTime() + COOLDOWN_MS); const reviewAvailable = index === latestIndex && !isAdmin && Date.now() < expiresAt.getTime() && attempt.review; return { date: attempt.date, overall: Number(attempt.overall), recruitScore: Number(attempt.recruit_score), archetype: attempt.archetype, scores: attempt.scores, ...(reviewAvailable ? { review: attempt.review } : {}) }; }), bestOverall: attempts.reduce((best, attempt) => Math.max(best, Number(attempt.overall)), 0), bestRecruitScore: attempts.reduce((best, attempt) => Math.max(best, Number(attempt.recruit_score)), 0) };
}

function validateAttempt(attempt: any): ValidatedAttempt | null {
  const overall = Number(attempt?.overall), recruitScore = Number(attempt?.recruitScore);
  const archetype = typeof attempt?.archetype === "string" ? attempt.archetype.trim().slice(0, 100) : "";
  const scores = attempt?.scores, review = attempt?.review ?? null;
...
export async function GET() {
  try {
    await ensureSchema(); const user = await getCurrentUser();
    if (!user) return NextResponse.json({ error: "Not authenticated." }, { status: 401 });
    const db = getDb(); const attempts = await db`SELECT date, overall, recruit_score, archetype, scores, review FROM attempts WHERE user_id = ${user.id} ORDER BY date ASC`;
...
export async function POST(request: Request) {
  try {
    ...
    const attempt = validateAttempt(body?.attempt);
    if (!attempt) return NextResponse.json({ error: "Invalid assessment result." }, { status: 400 });
    const db = getDb();
    const existing = await db`SELECT date FROM attempts WHERE user_id = ${user.id} ORDER BY date DESC LIMIT 1`;
    const latest = existing[0]?.date ? new Date(existing[0].date).getTime() : null;
    const isAdmin = String(user.username).toLowerCase() === ADMIN_USERNAME;
