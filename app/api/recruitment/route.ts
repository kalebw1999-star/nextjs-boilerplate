  ...
  try {
    ...
    if (playerId) {
      const rows = await db`
        ...
        WHERE u.id = ${playerId}
        GROUP BY u.id, u.username, u.created_at, rs.status, rs.updated_at, ac.cooldown_reset_at
      `;
      if (!rows.length) return NextResponse.json({ error: "Player not found." }, { status: 404 });

      const attempts = await db`
        SELECT id, date, overall, recruit_score, archetype, scores, review
        FROM attempts WHERE user_id = ${playerId} ORDER BY date DESC LIMIT 20
      `;
      const p = rows[0];
      return NextResponse.json({
        player: {
          id: p.id, username: p.username, createdAt: p.created_at,
          status: p.status ?? "none", statusUpdatedAt: p.status_updated_at ?? null,
          cooldownResetAt: p.cooldown_reset_at ?? null,
...
export async function POST(request: Request) {
  try {
    ...
    if (!playerId || !["reset_cooldown", "add_waiting", "add_team", "remove_team"].includes(action)) {
      return NextResponse.json({ error: "Invalid recruitment action." }, { status: 400 });
    }
    const db = getDb();
    if (action === "reset_cooldown") {
      await db`INSERT INTO assessment_controls (user_id, cooldown_reset_at) VALUES (${playerId}, NOW())
        ON CONFLICT (user_id) DO UPDATE SET cooldown_reset_at = NOW()`;
    } else {
      const status = action === "add_waiting" ? "waiting" : action === "add_team" ? "team" : "none";
